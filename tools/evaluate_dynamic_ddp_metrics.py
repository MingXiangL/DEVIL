...
def parse_args():
    ...
    parser.add_argument('--regress_model_weight_path', type=str, required=True)
...
def build_dinov2(model_name, rank):
    # model = torch.hub.load('/root/.cache/torch/hub/facebookresearch_dinov2_main', model_name, source='local')
    # model = torch.hub.load('/root/paddlejob/workspace/env_run/output/DevilBench/facebookresearch', model_name, source='local')
    model = torch.hub.load('/root/paddlejob/workspace/env_run/output/liaomingxiang/DevilBench/facebookresearch', model_name, source='local')
    model = model.to(rank)
    model = nn.parallel.DistributedDataParallel(model, device_ids=[rank])
    model.eval()
    return model
...
def build_clip(model_name, rank):
    ...
    model_clip, process_clip = clip.load('/root/paddlejob/workspace/env_run/output/liaomingxiang/DevilBench/ckpts/ViT-L-14.pt', device=rank)
...
def cal_dino_segment_dist(model, video_data, video_lengths):
    ...
    # 处理提取的特征,例如保存到文件或进一步处理
    # norm = torch.norm(features, dim=1, p=2, keepdim=True)
...
def cal_dino_segment_dist_chunk(model, video_data, video_lengths, max_frames=100):
    ...
    for length in video_lengths:
        ...
        # Process each segment in batches to gather all features first
...
def cal_dynamics_scores(args):
    ...
    with torch.no_grad():
        for frames in dataloader:
            ...
            # ssim_dists, phash_dists = cal_ssim_dist_chunk(video_no_resize, org_videos, video_names, max_frames=100)
            # print(f'ssim_dists: {ssim_dists}, phash_dists: {phash_dists}')
            # print(f'ssim_dists_chunk: {ssim_dists_chunk}, phash_dists_chunk: {phash_dists_chunk}')
            # Inter-segment level metrics
            dino_segment_dist = cal_dino_segment_dist(model_dino_v2, video_data, video_lengths, )
            # dino_segment_dist = cal_dino_segment_dist_chunk(model_dino_v2, video_data, video_lengths, )
            # dino_frechet_dist = cal_frechet_distance(model_dino, video_data, video_lengths, [0.5, 0.25])
            ...
            # clip_var_dist, clip_features = cal_info_variance(model_clip.module.visual, torch.cat([process_clip(v.permute(0,3,1,2) / 255) for v in org_videos]).to(rank).to(torch.half), video_lengths)
            ...
            res = [{
                ...
                # "frechet_dist_dino": dino_frechet_dist[i],
    ...
    # temporal_entropy = cal_temp_entropy(dataset)
    ...
    if rank == 0:
        ...
        # json_to_xlsx(f'{args.save_dir}/merged.json', f'{args.save_dir}/merged.xlsx')
...
def cal_dynamics_metircs(args):
    ...
    # cal_dynamics_based_quality(args)
