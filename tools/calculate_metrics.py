...
# def intergrate_per_method_auto_bin(args, df_quality, df_dynamics, keys=['subject_consistency', 'background_consistency'], name_col='video_name'):
    ...
    video_names_dynamics_score, dynamic_total = get_overall_dynamic_scores(df_dynamics)
    scores = dict()
...
def calculate_dynamics_based_quality(args):
    ...
    df_quality_unique_sorted['naturalness'] -= df_quality_unique_sorted['naturalness'].min()
    df_quality_unique_sorted['naturalness'] /= df_quality_unique_sorted['naturalness'].max()
