...
# from quality_metrics import background_consistency, subject_consistency, motion_smoothness, calculate_naturalness_score
from quality_metrics import background_consistency, subject_consistency, motion_smoothness
...
def parse_args():
    ...
    parser.add_argument('--amt_config_path', type=str, default='vbench/third_party/amt/cfgs/AMT-S.yaml')
    parser.add_argument('--amt_ckpt_path', type=str, default='vbench/third_party/amt/ckpt/amt-s.pth')
    parser.add_argument('--clip_path', type=str, default='model_weight/ViT-B-32.pt')
...
def load_and_merge(pth_folder, save_folder):
    all_datas = []
    save_path =  os.path.join(save_folder, 'merged_results.xlsx')
...
def merge_excel_files(file1, file2, output_file):
    ...
    df1 = df1.sort_values(by='video_name').reset_index(drop=True)
    df2 = df2.sort_values(by='video_name').reset_index(drop=True)
...
def main_dist(args):
    ...
    motion_smooth_config = {'config': '/Workspace2/LiaoMingxiang/VBench-master/vbench/third_party/amt/cfgs/AMT-S.yaml', 
            'ckpt': '/home/LiaoMingxiang/Workspace2/DinaBench/vbench/third_party/amt/ckpt/amt-s.pth'}
    results = []
    with torch.no_grad():
...
def calculate_quality_scores(args):
    output_file = f'{args.save_dir}/.tmp/results_{args.rank}.pth'
    os.makedirs(os.path.dirname(output_file), exist_ok=True)
    clear_folder(os.path.dirname(output_file))
    video_list = find_mp4_files(args.video_dir)
    with open(output_file, 'w') as file:
    ...
    clip_model, preprocess =  clip.load(args.clip_path, args.rank)
    dino_config = {'repo_or_dir': 'facebookresearch/dino:main', 'source': 'github', 'model': 'dino_vitb16'}
    dino_model = torch.hub.load(**dino_config).to(args.rank)
    motion_smooth_config = {'config': args.amt_config_path, 
            'ckpt': args.amt_ckpt_path}
    results = []
    with torch.no_grad():
        _, bg_consistency = background_consistency(clip_model, preprocess, video_list, args.rank)
        _, sj_consistency = subject_consistency(dino_model, video_list, args.rank)
        _, smoothness = motion_smoothness(motion_smooth_config, video_list, args.rank)
        for video_path in video_list:
            video_name = os.path.basename(video_path)
            results.append({
                'video_name': video_name,
    ...
    torch.save(results, output_file)
    save_path = load_and_merge(f'{args.save_dir}/.tmp', args.save_dir)
    shutil.rmtree(f'{args.save_dir}/.tmp')
    return save_path
