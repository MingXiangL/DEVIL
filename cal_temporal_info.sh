...
# cal_temporal_info_single /Volumes/My-Passport/VideoGeneration/normalized_prompt_data/concatenated_dir $sigma

calculate_temporal_info "$input_video" "$output_folder"

# ffmpeg -i input.mp4 -c:v libx264 -preset veryslow -crf 23  -threads 1 -x264-params "no-psy=1:aq-mode=1:aq-strength=1.0:rc-lookahead=40:keyint=250:min-keyint=25:bframes=3:me=umh:subme=9:partitions=all:ref=5:b-adapt=2:direct=auto:trellis=2" output.mp4
# ffmpeg -i input.mp4 -c:v libx264 -preset veryslow -crf 23 -threads 1 -x264-params "no-psy=1:aq-mode=1:aq-strength=1.0:rc-lookahead=40:keyint=250:min-keyint=25:bframes=3:me=umh:subme=9:partitions=all:ref=5:b-adapt=2:direct=auto:trellis=2:seed=1" output.mp4
